...
import kotlinx.android.synthetic.main.activity_receiving_list.MissionList
import kotlinx.android.synthetic.main.activity_receiving_list.back
...
class ReceivingListActivity : Activity() {
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            bmobQuery.addWhereEqualTo("receiveB","true")
            bmobQuery.addWhereEqualTo("receiverID",userId)
            bmobQuery.addWhereEqualTo("completeR","false")
            bmobQuery.findObjects(object : FindListener<MissionTable>() {
                override fun done(missions: MutableList<MissionTable>?, ex: BmobException?) {
                    if (ex == null) {
                        Toast.makeText(this@ReceivingListActivity, "查询成功", Toast.LENGTH_LONG).show()
                        val adapter= ArrayAdapter(this@ReceivingListActivity,R.layout.list_item,data)
                        ...
                        Toast.makeText(this@ReceivingListActivity, ex.message, Toast.LENGTH_SHORT).show()
                    }
                }
            })
